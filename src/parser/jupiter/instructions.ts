...
export class InstructionParser {
  ...
  // Add new method to get slippage
  getSlippageBps(
    ...
    for (const instruction of instructions) {
      ...
      try {
        const hexIxData = bs58.decode(instruction.data).toString("hex");
        const ix = this.coder.instruction.decode(hexIxData, "hex");
        if (ix && this.isRouting(ix.name)) {
          return (ix.data as any).slippageBps?.toString();
        }
      } catch (error) {
      ...
      if (ix && this.isRouting(ix.name)) {
        ...
        const transferAuthority =
          instruction.accounts[
            this.getTransferAuthorityIndex(instructionName) ?? 0
          ].toString();
        const lastAccount =
  ...
  getInstructions(tx: ParsedTransactionWithMeta): PartialInstruction[] {
